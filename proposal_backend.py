...
def fix_services_table_column(doc):
    table = find_table_with_any_marker(
        doc, ["<SVC_ITEM1>", "Vendor will provide the following services", "<SVC_DESC1>"]
    )
    if table is None:
        return
    for row in table.rows:
...
def fill_pricing_table(doc, pricing_items):
    ...
    for item in pricing_items:
        row = table.add_row()
        set_cell_text(row.cells[0], item.get("description", ""))
        set_cell_text(row.cells[1], item.get("price", ""))
        total += safe_float(item.get("price", ""))
    return total


def fill_machines_table(doc, machines):
    table = find_table_with_any_marker(
        doc, ["<M1_NAME>", "Parameter Description", "Parameter Quantity"]
    )
    if table is None:
        return
    clear_table_rows(table, keep_rows=1)
    total_qty = total_umm = total_mps = 0.0
    ...
    set_cell_text(row.cells[2], int(total_qty) if total_qty == int(total_qty) else total_qty)
    set_cell_text(row.cells[3], int(total_umm) if total_umm == int(total_umm) else total_umm)
    set_cell_text(row.cells[4], int(total_mps) if total_mps == int(total_mps) else total_mps)


def fill_bom_table(doc, bom_items):
    table = find_table_with_any_marker(doc, ["<MPS_I1>", "Part Number"])
    if table is None:
        return
    clear_table_rows(table, keep_rows=1)
...
def update_product_assumptions_table(doc, product_assumptions):
    table = find_table_with_any_marker(
        doc, ["Local PI Server Hardware", "PI System Access (PSA) for SETPOINT"]
    )
    if table is None:
        return
    incoming = {
...
def fill_scope_services_table(doc, scope_services):
    ...
    for i, item in enumerate(scope_services):
        row = table.add_row()
        set_cell_text(row.cells[0], "Services" if i == 0 else "")
        set_cell_text(row.cells[1], item.get("item", ""))
        set_cell_text(row.cells[2], item.get("description", ""))
...
def generate_proposal(data):
    ...
    # Remove yellow highlight and fix narrow Group column
    remove_highlight_all(doc)
    fix_services_table_column(doc)
