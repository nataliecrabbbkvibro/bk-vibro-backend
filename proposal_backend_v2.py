...
def create_proposal_doc(data):
    ...
    title.add_run("Proposal for the\n").font.size = Pt(12)
    title_main = title.add_run("Buyer Plant Machine Trains\n")
    title_main.font.size = Pt(18)
    title_main.bold = True
    
    doc.add_paragraph("SETPOINT™ Machinery Protection Systems").alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("And Condition Monitoring Software").alignment = WD_ALIGN_PARAGRAPH.CENTER
    
    doc.add_paragraph()  # Space
    
    # Buyer Contact
    doc.add_paragraph("Buyer Contact:").bold = True
    ...
    # Sales Contact
    doc.add_paragraph("Brüel & Kjær Vibro Contact:").bold = True
    ...
    for item in data.get("pricingItems", []):
        ...
        row_cells[1].text = item.get("price", "")
        
        try:
            price_val = float(str(item.get("price", "0")).replace("$", "").replace(",", ""))
    ...
    # BOM tables by group
    bom_groups = data.get("bomItems", [])
    for group_name in set([item.get("group", "") for item in bom_groups]):
        doc.add_paragraph(f"{group_name} Group").bold = True
        ...
        for item in bom_groups:
            if item.get("group") == group_name:
                ...
                row_cells[2].text = item.get("partNumber", "")
    ...
    doc.add_paragraph(
        ...
        "comply with API 670 requirements for machinery protection systems. Up to 56 vibration/position/speed "
        "channels or 84 temperature channels can be monitored and displayed in a single 19\" rack."
    )
    ...
    doc.add_paragraph(
        ...
        "Implementations can include PI-integrated (CMSPI) or non-PI configurations depending on customer requirements."
    )
    ...
    doc.add_paragraph("Shipment Terms: Ex Works Minden, NV USA")
    ...
    doc.add_paragraph(data.get("salesTitle", "Regional Manager"))
